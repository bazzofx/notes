Below are a list of ==special variables== on #powershell that are used and known by the operating system.
...
| `$MYINVOCATION`    | Populated only for scripts, functions, and script blocks. `PSScriptRoot` and `PSCommandPath` properties of the `$MyInvocation` automatic variable contain information about the invoker or calling script, not the current script. |
